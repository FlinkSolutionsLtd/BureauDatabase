CREATE procedure [bureau].[sp_get_staffing_test]
	 @start datetime
	,@hospital int
as
...
from	
	...
	select
		...
		,shift_date
		,shift_code
		,sum(case when ds_type = 'ACTUAL' then RN + Coord end) as RN
		,sum(case when ds_type = 'ACTUAL' then EN end) as EN
		,sum(case when ds_type = 'ACTUAL' then HCA end) as HCA
		,sum(case when ds_type = 'BUDGET' then RN + Coord end) as RN_Bud
			...
			(
				...
				and t.shift_date = 
					(
						...
						where 
							 ds_type = 'BUDGET'
							 and bud.loc_id = t.loc_id
						order by 
							 abs(datediff(day,@start,bud.shift_date))
					)
			)
		)
	group by
